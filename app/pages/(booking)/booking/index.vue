        ...
        <div class="p-6 grid grid-cols-1 md:grid-cols-2 justify-center gap-x-4">
            ...
            <div class="mt-4 md:mt-0 w-full md:w-auto">
              <ModuleDropdown label="ROOMS"
                ...
                <UiInputNumber id="rooms-quantity-input" v-model="formFilter.rooms" label="Rooms" name="rooms"
                  :min="1" />
                <UiInputNumber id="adult-quantity-input" v-model="formFilter.adults" label="Adult" name="adult"
                  :min="1" />
          ...
          <div class="w-full md:w-auto mt-4 md:mt-0">
            ...
            <div class="w-auto flex flex-wrap gap-2">
              <div
                :class="(morePackage ? '' : 'max-h-12 overflow-hidden') + ' flex flex-wrap gap-2 md:max-w-2/3 items-end'">
                <div v-for="(packages, index) in dataPackages" :key="index">
                  <UiRadioBox :id="'packages' + packages.id" v-model="packages.name" name="room_package">{{
                    packages.name }}
                    <UIcon v-if="packages.favorite" name="material-symbols:star" class="size-5 ml-1 text-yellow-600" />
                  </UiRadioBox>
                </div>
                <div v-if="morePackage" class="max-w-18">
                  <label :class="['text-xs font-semibold cursor-pointer text_theme']"
                    @click="morePackage = !morePackage"> Hide Packages...</label>
                </div>
              </div>
              <div v-if="!morePackage" class="md:max-w-18 md:-ml-2">
                <label :class="['text-xs font-semibold cursor-pointer text_theme']" @click="morePackage = !morePackage">
                  More Packages...</label>
              </div>
            </div>
          </div>
        ...
        <div :class="'flex flex-col space-y-3 ' + ((carts.length > 0) ? 'md:w-2/3' : 'w-full')">
          <ModuleCard v-for="room in roomData" :key="room.id">
            ...
            <p :class="'text-xs flex md:hidden items-end font-light text-slate-400 text_theme'">
              ...
              <UIcon name="material-symbols-light:king-bed-outline" class="size-5 ml-3 mr-1" /> {{ room.category }} - {{
                room.bed }}
            </p>
            <!-- Carousel Image -->
            ...
            <div class="md:w-2/3 md:grid md:grid-cols-3 md:gap-2 md:ml-6">
              ...
              <div :class="['md:border-r md:border_theme']">
                ...
                <a class="font-light text-xs inline-flex cursor-pointer text-green-500 underline"
                  data-drawer-target="drawer-description" data-drawer-show="drawer-description"
                  data-drawer-placement="right" aria-controls="drawer-description" data-drawer-backdrop="false"
                  @click="toggleMoreFacilities(room)">More Info ...
                  <UIcon name="material-symbols:arrow-outward" class="size-4" />
                </a>
              </div>
              <!-- Package Price -->
              <div class="w-full flex flex-col md:col-span-2">
                <div class="flex justify-end mb-3 md:hidden">
                  <UiButton variant="theme" className="text-left items-center" @click="toggleMorePackages(room.id)">
                    <p v-if="lowestPrice(room.packages).initial_price !== lowestPrice(room.packages).price"
                      class="text-xs font-light mb-1 line-through">IDR {{
                        formatPrice(lowestPrice(room.packages).initial_price) }}</p>
                ...
                <div v-for="packages in room.packages" :key="packages.id"
                  :class="(morePackages.includes(room.id) ? '' : 'hidden') + ' p-2 border-t border-slate-300 md:flex md:w-full'">
                  <div class="grid grid-cols-2 md:grid-cols-4 gap-y-2 md:w-full">
                    <div class="md:order-1">
                      ...
                      <p :class="['font-thin text-2xs mb-0 text_theme']">
                        <UIcon name="material-symbols:vertical-shades-closed" class="size-3 mr-1" />Cancelation Polish
                        Apply
                      </p>
                      <p :class="['font-thin text-2xs mb-0 text_theme']">
                        <UIcon name="streamline-block:drink-food-food" class="size-3 mr-1" />Breakfast {{
                          packages.breakfast ? 'Include' : 'Exclude' }}
                      </p>
                    </div>
                    <div class="text-right md:order-3">
                      <p v-if="packages.initial_price !== packages.price"
                        class="text-xs font-light text-red-600 line-through mb-0">IDR {{
                          formatPrice(packages.initial_price) }}</p>
                      ...
                      <p :class="['text-[9px] font-thin mb-0 text_theme']">Tax {{ packages.tax ? 'Include' : 'Exclude'
                      }}</p>
                    </div>
                    <div v-if="packages.stock - packages.booking > 0" class="w-15 md:order-2 md:justify-self-end">
                      <UiSelect id="quantity" v-model="packages.quantity" label="Rooms"
                        :options="packages.stock - packages.booking" placeholder="0" labelClass="text_theme"
                        inputClass="border_theme focus: '+color.border_theme focus:ring_theme" />
                    </div>
                    ...
                    <div class="text-right md:order-4">
                      <UiButton v-if="packages.stock - packages.booking > 0" valid class="ml-2 w-full" variant="theme"
                        @clicked="addToCart(room.id, packages.id, packages.quantity); packages.quantity = 0">
                        {{ packages.booking > 0 ? 'Book More' : 'Book' }}
                      </UiButton>
                      <p v-if="packages.booking > 0" class="text-xs font-thin text-green-500 mt-1 text-right">{{
                        packages.booking }} Room Selected</p>
                    </div>
                  </div>
                </div>
              </div>
