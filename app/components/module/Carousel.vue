...
<template>
  <div :id="id" :class="['relative w-full md:w-1/3 mb-2 md:-mt-2', className]" data-carousel="slide">
    <div :class="['relative h-56 overflow-hidden rounded-base md:h-72', subClassName]">
      <div v-for="(image, index) in images" :key="index" class="hidden duration-700 ease-in-out" data-carousel-item>
        <NuxtImg
          ...
          :src="image"
          :alt="index"
          :class="['object-cover w-full rounded-base h-60 md:h-auto my-3 md:mb-0', imageClassName]"
          :custom="true"
          format="webp"
        >
    ...
    <div v-if="indicators" class="absolute z-30 flex -translate-x-1/2 bottom-5 left-1/2 space-x-3 rtl:space-x-reverse">
      <button v-for="(image, index) in images" :key="index" type="button" class="w-3 h-3 rounded-base" :aria-current="index === 0 ? 'true' : 'false'" :aria-label="'Slide ' + index + ''" :data-carousel-slide-to="index"></button>
    </div>
