...
<template>
  <div>
    <!-- Cart -->
    <FragmentCard :class="['', (isPayment && 'mt-3 mb-20')]">
      <div class="flex flex-col space-x-2 mb-8 w-full">
        <h4 :class="['text_theme font-bold text-base mb-4']">Booking Details</h4>
        <div class="flex flex-row space-x-6 border-b border-default mb-2 pb-3">
          <div class="">
            <p class="text-[12px] font-semibold">Check In</p>
            <p class="text-[12px] font-semibold">{{ dateFormatter('2026-05-12') }} </p>
          </div>
          <div class="">
            <p class="text-[12px] font-semibold">Check Out</p>
            <p class="text-[12px] font-semibold">{{ dateFormatter('2026-05-14') }} </p>
          </div>
        </div>
          ...
          <table class="w-full">
            <tbody>
              <tr v-for="detail in cart.details" :key="detail.id">
                <th
                  :class="['py-1 text-green-500 font-medium text-[13px] text-start', (isPayment ? 'w-[60%]' : 'w-[50%]')]">
                  {{ detail.name }}</th>
                <th class="py-1 text-amber-950 font-semibold text-[13px] text-end w-[40%]">
                  IDR {{
              ...
              <tr>
                <th
                  :class="['py-1 text-slate-800 font-bold text-[13px] text-start', (isPayment ? 'w-[60%]' : 'w-[50%]')]">
                  Subtotal</th>
                <th class="py-1 text-slate-800 font-bold text-[13px] text-end w-[40%]">
                  IDR {{
                    formatPrice(cart.subtotal) }}</th>
                <th v-if="!isPayment"></th>
              </tr>
            </tbody>
          </table>
        </div>
        ...
        <UiButton v-if="!isPayment" class="w-full mt-4" variant="theme" @click="handleCheckout">
          Complete Booking
        </UiButton>
        <div v-if="isPayment">
          <div class="flex flex-col space-y-2 mb-5">
            <h5 class="font-bold text-base text-slate-800">Payment Type</h5>
            <div class="w-full flex flex-row border items-center justify-between border-slate-800 p-3 rounded-sm">
              <div class="flex">
                <NuxtImg src="/images/payment/briva.png" alt="BRI VA" class="mr-3 h-7" loading="lazy" />
                <p class="text-slate-800 text-sm font-semibold">BRI Virtual Account</p>
              </div>
              <UiButton variant="custom" @clicked="openModal">Change
              </UiButton>
            </div>
            <div class="mt-3">
              <UiCheckBox id="terms" name="terms">
                By booking, you have agreed to our <a href="#" class="text-green-500">Terms & Conditions</a>
              </UiCheckBox>
              <UiCheckBox id="subscribe" name="subcribe">
                Subscribe to Eaze Villas Uluwatu - Bali Newsletter
              </UiCheckBox>
            </div>
          </div>
          <UiButton class="w-full mt-4" variant="theme">
            Pay Now
          </UiButton>
          <NuxtLink to="/booking?data=uluwatu">
            <UiButton class="w-full mt-4" variant="outline">
              Change Order
            </UiButton>
          </NuxtLink>
        </div>
    ...
    <!-- Cart Mobile -->
    <div v-if="!isDesktop && !isPayment">
      <button
        class="fixed bottom-6 right-6 h-12 w-12 flex justify-center items-center text-white p-4 rounded-full shadow-lg hover:opacity-90 focus:outline-none z-100 transition-all duration-300 bg_theme animate-bounce"
    ...
    <!-- Drawer Cart -->
    <FragmentDrawer v-if="!isDesktop && !isPayment" id="drawer-cart" position="bottom" title="Booking Details"
      className="text_theme">
      <div class="flex flex-col space-x-2 mb-8">
        <div class="flex flex-row space-x-6 border-b border-default mb-2 pb-3">
          <div class="">
            <p class="text-xs font-medium">Check In</p>
            <p class="text-xs font-medium">{{ dateFormatter('2026-05-12') }} </p>
          </div>
          <div class="">
            <p class="text-xs font-medium">Check Out</p>
            <p class="text-xs font-medium">{{ dateFormatter('2026-05-14') }} </p>
          </div>
        </div>
        <div v-for="cart in carts" :key="cart.id" class="flex flex-col space-y-2 border-b border-default mb-2 pb-3">
          <h4 class="font-semibold text-[16px] mb-2">{{ cart.name }}</h4>
    ...
    <!-- Payment Method -->
    <FragmentModal v-if="isPayment" size="md" title="Payment Method">
      <p class="text-sm">Lorem ipsum dolor sit amet consectetur adipisicing elit. Aperiam suscipit quae consectetur
      </p>
      <ul class="my-4 space-y-3">
        <li>
          <a href="#"
            ...
            <span
              class="bg-neutral-secondary-medium border border-default-medium text-heading text-xs font-medium px-1.5 py-0.5 rounded">Popular</span>
          </a>
        </li>
        <li>
          <a href="#"
        ...
        <li>
          <a href="#"
            class="flex items-center p-4 text-base font-semibold text-heading rounded-base border border-default-medium hover:border-brand-subtle bg-neutral-secondary-medium hover:bg-brand-softer hover:text-fg-brand">
            <svg aria-hidden="true" class="h-5" viewBox="0 0 512 512" version="1.1" xmlns="http://www.w3.org/2000/svg"
              xmlns:xlink="http://www.w3.org/1999/xlink">
              <defs>
      ...
      <div>
        <a href="#" class="inline-flex items-center text-xs font-normal text-body hover:underline">
          <svg class="w-3.5 h-3.5 me-1" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24"
